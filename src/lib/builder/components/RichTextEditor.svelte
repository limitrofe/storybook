...
<div class="rt-wrapper">
  <div class="rt-toolbar" role="toolbar" aria-label="Ferramentas de formatação">
    <button type="button" on:mousedown|preventDefault on:click={() => exec('bold')} title="Negrito (Ctrl/Cmd + B)"><strong>B</strong></button>
    <button type="button" on:mousedown|preventDefault on:click={() => exec('italic')} title="Itálico (Ctrl/Cmd + I)"><em>I</em></button>
    <button type="button" on:mousedown|preventDefault on:click={() => exec('underline')} title="Sublinhado (Ctrl/Cmd + U)"><u>U</u></button>
    <button type="button" on:mousedown|preventDefault on:click={() => exec('strikeThrough')} title="Tachado">S</button>
    <span class="rt-divider" aria-hidden="true"></span>
    <button type="button" class:active={isUnorderedListActive} on:mousedown|preventDefault on:click={() => exec('insertUnorderedList')} title="Lista com marcadores">•≡</button>
    <button type="button" class:active={isOrderedListActive} on:mousedown|preventDefault on:click={() => exec('insertOrderedList')} title="Lista numerada">1.</button>
    <button type="button" on:mousedown|preventDefault on:click={insertLink} title="Inserir link">🔗</button>
    <span class="rt-divider" aria-hidden="true"></span>
    <label class="rt-color" title="Cor do texto">
      <input type="color" on:mousedown={(event) => { event.preventDefault(); editorEl?.focus(); }} on:input={(event) => applyColor(event.target.value)} />
    </label>
    <button type="button" on:mousedown|preventDefault on:click={() => exec('removeFormat')} title="Remover formatação">⟲</button>
  </div>
