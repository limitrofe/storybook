...
<header class="toolbar">
  ...
  <div class="actions">
    <button class="ghost" on:click={handleNewStory} title="Limpar história">
      Novo
    </button>
    <button class="ghost" on:click={handleReload} title="Carregar story.json salvo">
      Recarregar
    </button>
    <button class="primary" on:click={handleSave} title="Salvar story.json">
      Salvar
    </button>
    <button class="ghost" on:click={handleExport} title="Exportar JSON">
      Exportar
    </button>
    <button class="ghost" on:click={handleImportClick} title="Importar JSON">
      Importar
    </button>
...
<style>
  .toolbar {
    ...
    gap: 1.25rem;
    padding: 1rem 2rem;
    background: linear-gradient(120deg, rgba(30, 64, 175, 0.85) 0%, rgba(29, 78, 216, 0.8) 45%, rgba(14, 165, 233, 0.75) 100%);
    color: white;
