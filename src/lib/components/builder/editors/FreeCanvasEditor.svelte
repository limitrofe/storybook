...
function setCanvasValue(field, value) {
  ...
  function getDragOptions(item) {
    let origin = { x: item[currentDevice]?.x || 0, y: item[currentDevice]?.y || 0 };
    return {
      ...
      onDragStart: () => {
        origin = { x: item[currentDevice]?.x || 0, y: item[currentDevice]?.y || 0 };
        selectedId = item.id;
        updatePreviewRect();
      },
      onDrag: ({ offsetX, offsetY }) => {
        const nextX = clampPosition(origin.x + offsetX / (scale || 1), 'x', item);
        const nextY = clampPosition(origin.y + offsetY / (scale || 1), 'y', item);
        updateItem(item.id, (draft) => {
...
<div class="editor">
  <header class="toolbar">
    <div class="device">
      <button type="button" class:active={currentDevice === 'desktop'} on:click={() => selectDevice('desktop')}>
        Desktop
      </button>
      <button type="button" class:active={currentDevice === 'mobile'} on:click={() => selectDevice('mobile')}>
        Mobile
      </button>
    </div>
    <div class="actions">
      ...
      <section class="panel">
        ...
        <label>
          ...
          <input
            ...
            min="0"
            placeholder="auto"
            on:input={(e) => setCanvasValue('maxHeightDesktop', e.currentTarget.value === '' ? null : Number(e.currentTarget.value))}
          />
        </label>
        <label>
        ...
        <label>
          ...
          <input
            ...
            min="0"
            placeholder="auto"
            on:input={(e) => setCanvasValue('maxHeightMobile', e.currentTarget.value === '' ? null : Number(e.currentTarget.value))}
          />
        </label>
        <label>
          Cor de fundo
          <input type="color" value={data.backgroundColor} on:input={(e) => { data = { ...data, backgroundColor: e.currentTarget.value }; emit(); }} />
        </label>
        <label>
          Largura base desktop (px)
          <input type="number" min="320" value={data.baseWidthDesktop} on:input={(e) => setCanvasNumber('baseWidthDesktop', e.currentTarget.value)} />
        </label>
        <label>
          Largura base mobile (px)
          <input type="number" min="200" value={data.baseWidthMobile} on:input={(e) => setCanvasNumber('baseWidthMobile', e.currentTarget.value)} />
        </label>
      </section>
      ...
      <section class="panel">
        ...
        {#if selectedItem}
          ...
          <div class="grid">
            <label>
              X ({currentDevice})
              <input type="number" value={selectedItem[currentDevice].x} on:input={(e) => setFrameValue(selectedItem, currentDevice, 'x', e.currentTarget.value)} />
            </label>
            <label>
              Y ({currentDevice})
              <input type="number" value={selectedItem[currentDevice].y} on:input={(e) => setFrameValue(selectedItem, currentDevice, 'y', e.currentTarget.value)} />
            </label>
            <label>
              Largura ({currentDevice})
              <input type="number" value={selectedItem[currentDevice].width} min="0" on:input={(e) => setFrameValue(selectedItem, currentDevice, 'width', e.currentTarget.value)} />
            </label>
            <label>
          ...
          <label>
            Conteúdo (HTML)
            <textarea rows="4" value={selectedItem.content} on:input={(e) => { updateItem(selectedItem.id, (draft) => (draft.content = e.currentTarget.value)); emit(); }}></textarea>
          </label>
          ...
          {#if selectedItem.type === 'text'}
            <label>
              Cor do texto
              <input type="color" value={selectedItem.textStyles?.color || '#ffffff'} on:input={(e) => setTextValue('color', e.currentTarget.value)} />
            </label>
            <label>
              Tipografia (usa HTML automático)
              <select value={selectedItem.textStyles?.typography || ''} on:change={(e) => { setTextValue('typography', e.currentTarget.value); }}>
