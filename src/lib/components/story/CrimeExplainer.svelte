...
>
      ...
      {#each crimes as crime, index}
        ...
        >
          ...
          <div class="severity-indicator">
            <div 
              class="severity-bar"
              style="width: {calculateSeverityWidth(crime)}%; background: {getSeverity(crime).color}"
            ></div>
          ...
          <div class="card-content">
            ...
            <div class="crime-meta">
              <div class="legal-base">
                <strong>Base Legal:</strong> {crime.legalBase}
              </div>
              ...
              {#if showPenalties}
                <div class="penalty-range">
                  ...
                  {#if crime.hasAggravants}
                    <span class="aggravants">(até {formatPenalty(crime.penaltyMaxWithAggravants)} com agravantes)</span>
                  {/if}
                </div>
              {/if}
            </div>
          ...
          <div class="details-section penalty-analysis">
            ...
            <div class="penalty-breakdown">
              <div class="penalty-base">
                <strong>Pena Base:</strong> {formatPenalty(selectedCrime.penaltyMin)} a {formatPenalty(selectedCrime.penaltyMax)}
              </div>
              ...
              {#if selectedCrime.aggravants && selectedCrime.aggravants.length > 0}
                <div class="aggravants-section">
                  ...
                  <div class="max-penalty">
                    <strong>Pena Máxima com Agravantes:</strong> {formatPenalty(selectedCrime.penaltyMaxWithAggravants)}
                  </div>
                </div>
              {/if}
            </div>
            ...
            <div class="severity-visualization">
              <div class="severity-scale">
                <div class="scale-bar">
                  <div 
                    class="scale-fill"
                    style="width: {calculateSeverityWidth(selectedCrime)}%; background: {getSeverity(selectedCrime).color}"
                  ></div>
                </div>
              ...
              <div class="severity-label">
                {getSeverity(selectedCrime).icon} {getSeverity(selectedCrime).label}
              </div>
            </div>
          </div>
        {/if}
            ...
            <ul class="precedents-list">
              {#each selectedCrime.precedents as precedent}
                <li class="precedent-item">
                  <strong>{precedent.case}:</strong> {precedent.outcome}
                </li>
              {/each}
            </ul>
          </div>
  ...
  <div class="crimes-summary">
      ...
      {#if crimes.length > 0}
        <div class="stat-item">
          <span class="stat-number">
            {formatPenalty(Math.max(...crimes.map(c => c.penaltyMaxWithAggravants || c.penaltyMax)))}
          </span>
    ...
    <div class="summary-note">
      <p class="note-text">
        <strong>Importante:</strong> As penas podem ser aplicadas de forma cumulativa, 
        podendo resultar em condenação total superior à pena máxima individual de cada crime.
      </p>
    </div>
  </div>
</div>
