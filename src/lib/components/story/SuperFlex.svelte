<script>
	...
	function hasResponsiveText(value) {
		...
		if (typeof value === 'object') {
			return DEVICES.some((device) => {
				const deviceValue = value[device];
				return (
					deviceValue !== undefined && deviceValue !== null && String(deviceValue).trim().length > 0
				);
			});
		}
	...
	function resolveContainerBackground(containerConfig) {
		if (!containerConfig || typeof containerConfig !== 'object') {
			return { ...DEFAULT_BACKGROUND };
		}

		const background =
			containerConfig.background && typeof containerConfig.background === 'object'
				? containerConfig.background
				: {};

		const image = background.image ??
			containerConfig.backgroundImage ?? {
				mobile: containerConfig.backgroundImageMobile,
				tablet: containerConfig.backgroundImageTablet,
				desktop: containerConfig.backgroundImageDesktop ?? containerConfig.backgroundImage,
				wide: containerConfig.backgroundImageWide
			};

		const video = background.video ??
			containerConfig.backgroundVideo ?? {
				mobile: containerConfig.backgroundVideoMobile,
				tablet: containerConfig.backgroundVideoTablet,
				desktop: containerConfig.backgroundVideoDesktop ?? containerConfig.backgroundVideo,
				wide: containerConfig.backgroundVideoWide
			};

		return getBackground({
			background: {
				...background,
				color: background.color ?? containerConfig.backgroundColor,
				overlayColor: background.overlayColor ?? containerConfig.overlayColor,
				overlayOpacity: background.overlayOpacity ?? containerConfig.overlayOpacity,
				image,
				video
			}
		});
	...
	function getBackground(item) {
		...
		const background = {
			...
			image: toResponsiveObject(
				config.image ?? { desktop: config.imageDesktop, mobile: config.imageMobile },
				'',
				''
			),
			video: toResponsiveObject(
				config.video ?? { desktop: config.videoDesktop, mobile: config.videoMobile },
				'',
				''
			),
			size: toResponsiveObject(
				config.size ?? { desktop: config.imageSizeDesktop, mobile: config.imageSizeMobile },
				'cover',
				'cover'
			),
			position: toResponsiveObject(
				config.position ?? {
					desktop: config.imagePositionDesktop,
					mobile: config.imagePositionMobile
				},
				'center center',
				'center center'
			),
			repeat: toResponsiveObject(
				config.repeat ?? { desktop: config.imageRepeatDesktop, mobile: config.imageRepeatMobile },
				'no-repeat',
				'no-repeat'
			),
		...
		background.image.tablet =
			background.image.tablet || background.image.desktop || background.image.mobile;
		background.image.wide =
			background.image.wide || background.image.desktop || background.image.tablet;
		background.video.tablet =
			background.video.tablet || background.video.desktop || background.video.mobile;
		background.video.wide =
			background.video.wide || background.video.desktop || background.video.tablet;
	...
	function buildBaseStyle(item, extraVars = {}) {
		...
		assignResponsive(styleEntries, '--item-background-image', {
			...
			tablet: background.image.tablet
				? toCssUrl(background.image.tablet)
				: background.image.desktop
					? toCssUrl(background.image.desktop)
					: 'none',
			desktop: background.image.desktop ? toCssUrl(background.image.desktop) : 'none',
			wide: background.image.wide
				? toCssUrl(background.image.wide)
				: background.image.desktop
					? toCssUrl(background.image.desktop)
					: 'none'
		});
		...
		assignResponsive(
			...
			collectResponsiveValues(background.size, 'cover')
		);
		assignResponsive(
			styleEntries,
			'--item-background-position',
			collectResponsiveValues(background.position, 'center center')
		);
		assignResponsive(
			styleEntries,
			'--item-background-repeat',
			collectResponsiveValues(background.repeat, 'no-repeat')
		);

		styleEntries.push(`--item-background-color:${background.color || 'transparent'}`);
		styleEntries.push(
			`--item-background-overlay-color:${background.overlayColor || 'transparent'}`
		);
	...
	$: containerWidth = collectResponsiveValues(container.width || data?.width, '100vw');
	$: containerFlexDirection = collectResponsiveValues(
		container.flexDirection || data?.flexDirection,
		'column'
	);
	$: containerAlignItems = collectResponsiveValues(
		container.alignItems || data?.alignItems,
		'center'
	);
	$: containerJustifyContent = collectResponsiveValues(
		container.justifyContent || data?.justifyContent,
		'center'
	);
	$: containerBackgroundSource = { ...(data || {}), ...(container || {}) };
	$: containerBackground = resolveContainerBackground(containerBackgroundSource);
	$: containerBackgroundImages = toBackgroundImages(containerBackground.image);
	$: containerBackgroundVideos = containerBackground.video;
	$: containerBackgroundColor =
		containerBackground.color ||
		container.backgroundColor ||
		data?.backgroundColor ||
		'transparent';
	$: containerOverlayColor = containerBackground.overlayColor || 'transparent';
	$: containerOverlayOpacity = containerBackground.overlayOpacity || '0';
	$: containerHasBackgroundVideo = Boolean(
		containerBackgroundVideos.mobile ||
			containerBackgroundVideos.tablet ||
			containerBackgroundVideos.desktop ||
			containerBackgroundVideos.wide
	);
	$: containerHasBackgroundOverlay = Boolean(
		containerOverlayColor &&
			containerOverlayColor !== 'transparent' &&
...
<section
	...
	style:--container-width-mobile={containerWidth.mobile}
	style:--container-width-tablet={containerWidth.tablet}
	style:--container-width-desktop={containerWidth.desktop}
	style:--container-width-wide={containerWidth.wide}
	style:--container-overlay-color={containerOverlayColor}
	style:--container-overlay-opacity={containerOverlayOpacity}
...
>
	{#if containerHasBackgroundVideo || containerHasBackgroundOverlay}
		<div class="container-background">
			{#if containerHasBackgroundVideo}
				<video class="container-background-video" autoplay loop muted playsinline>
					{#if containerBackgroundVideos.mobile}
						<source
							src={containerBackgroundVideos.mobile}
							type="video/mp4"
							media="(max-width: 767px)"
						/>
					{/if}
					{#if containerBackgroundVideos.tablet}
						<source
							src={containerBackgroundVideos.tablet}
							type="video/mp4"
							media="(min-width: 768px) and (max-width: 1199px)"
						/>
					{/if}
					{#if containerBackgroundVideos.desktop}
						<source
							src={containerBackgroundVideos.desktop}
							type="video/mp4"
							media="(min-width: 1200px) and (max-width: 1599px)"
						/>
					{/if}
					{#if containerBackgroundVideos.wide}
						<source
							src={containerBackgroundVideos.wide}
							type="video/mp4"
			...
			{/if}
			{#if containerHasBackgroundOverlay}
				<div class="container-background-overlay"></div>
			{/if}
		</div>
	{/if}

	<div class="super-flex-content">
					...
					<div class={`flex-item type-${item.type || 'custom'}`} style={buildBaseStyle(item)}>
						...
						<div class="item-content">
							{#if item.type === 'text'}
								...
								{#if item.grifo}
									<img class="grifo-decorator" src={item.grifo.src} alt="" />
								{/if}
							{:else if item.type === 'image' && (item.src || item.content)}
								{@const imageSources = toResponsiveObject(item.src ?? item.content, '', '')}
								...
								{#if hasImageSource}
									{@const hasCaption = hasResponsiveText(item.caption)}
									{@const hasCredit = hasResponsiveText(item.credit)}
									{@const captionValues = collectResponsiveValues(item.caption, '')}
									{@const creditValues = collectResponsiveValues(item.credit, '')}
									<figure class="media-figure image-figure">
										{#if item.link}
											<a
												href={item.link}
											...
											>
												<picture class="image-element">
													{#if imageSources.mobile}
														<source media="(max-width: 767px)" srcset={imageSources.mobile} />
													{/if}
													{#if imageSources.tablet}
														<source
															media="(min-width: 768px) and (max-width: 1199px)"
															srcset={imageSources.tablet}
														/>
													{/if}
													{#if imageSources.desktop}
														<source
															media="(min-width: 1200px) and (max-width: 1599px)"
															srcset={imageSources.desktop}
														/>
													{/if}
													{#if imageSources.wide}
														<source media="(min-width: 1600px)" srcset={imageSources.wide} />
													{/if}
													<img
											...
											<figcaption>
												{#if hasCaption}
													{#each DEVICES as device}
														{#if captionValues[device]}
															<p class={`caption caption-${device}`}>{captionValues[device]}</p>
														{/if}
													{/each}
												{/if}
												{#if hasCredit}
													{#each DEVICES as device}
														{#if creditValues[device]}
															<p class={`credit credit-${device}`}>{creditValues[device]}</p>
														{/if}
													{/each}
												{/if}
											</figcaption>
								...
								{/if}
							{:else if item.type === 'video' && (item.src || item.content)}
								{@const videoSources = toResponsiveObject(item.src ?? item.content, '', '')}
								...
								{#if hasVideoSource}
									{@const posterSources = toResponsiveObject(item.poster, '', '')}
									{@const posterUrl =
										posterSources.desktop ||
										posterSources.tablet ||
										posterSources.mobile ||
										posterSources.wide}
									{@const hasCaption = hasResponsiveText(item.caption)}
									{@const hasCredit = hasResponsiveText(item.credit)}
									{@const captionValues = collectResponsiveValues(item.caption, '')}
											...
											>
												...
												>
													...
													{#if videoSources.desktop}
														<source
															src={videoSources.desktop}
															type="video/mp4"
															media="(min-width: 1200px) and (max-width: 1599px)"
														/>
													{/if}
													{#if videoSources.wide}
														<source
															src={videoSources.wide}
															type="video/mp4"
															media="(min-width: 1600px)"
														/>
													{/if}
...
<style>
	...
	.container-background-overlay {
		...
		opacity: var(--container-overlay-opacity, 0);
	}
	...
	@media (min-width: 768px) {
		...
		.flex-item {
			...
			background-size: var(
				--item-background-size-tablet,
				var(--item-background-size-mobile, cover)
			);
	...
	@media (min-width: 1200px) {
		...
		.flex-item {
			...
			background-size: var(
				--item-background-size-desktop,
				var(--item-background-size-tablet, cover)
			);
			background-position: var(
	...
	@media (min-width: 1600px) {
		...
		.flex-item {
			...
			background-image: var(
				--item-background-image-wide,
				var(--item-background-image-desktop, none)
			);
