...
<script>
  ...
  function shareToPlat form(platform, event) {
    if (platform === 'copy') {
      copyToClipboard();
      return;
    }
...
<div class="social-share social-share--{position} social-share--{theme}">
  <!-- Botão principal -->
  <button 
    class="social-share__trigger"
    on:click={nativeShare}
    title="Compartilhar esta história"
  >
      ...
      <div class="social-share__platforms">
        {#each platforms as platform}
          <button
            ...
            on:click={(e) => shareToPlat form(platform, e)}
