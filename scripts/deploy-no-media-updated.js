...
class DeployWithoutMedia {
  ...
  isMediaFile(filePath) {
    ...
    const isCodeFile = [...codeExtensions, ...fontExtensions].some(ext => lowerPath.endsWith(ext));
    ...
    for (let i = 0; i < files.length; i += batchSize) {
      ...
      await Promise.all(batch.map(async (file) => {
        try {
          ...
          process.stdout.write(`\r✅ ${uploaded}/${files.length} (${progress}%) - ${file.relativePath.substring(0, 40)}...`);
