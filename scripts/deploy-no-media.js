...
class DeployWithoutMedia {
  constructor(projectName) {
    ...
    this.mediaExtensions = [
      ...
      '.mp4', '.webm', '.mov', '.avi', '.mkv',                   // Vídeos
      '.mp3', '.wav', '.ogg', '.m4a',                            // Áudios
      '.pdf', '.zip', '.rar', '.tar', '.gz'                      // Outros binários
    ];
    ...
    for (let i = 0; i < files.length; i += batchSize) {
      ...
      await Promise.all(batch.map(async (file) => {
        try {
          ...
          process.stdout.write(`\r✅ ${uploaded}/${files.length} (${progress}%) - ${file.relativePath.substring(0, 50)}`);
  ...
  showResults() {
    console.log('\n🎉 DEPLOY CONCLUÍDO!');
    console.log('=' .repeat(50));
    console.log(`📤 Enviados: ${this.stats.uploaded} arquivos (${this.formatBytes(this.stats.uploadedSize)})`);
    console.log(`⏭️  Pulados: ${this.stats.skipped} mídias (${this.formatBytes(this.stats.skippedSize)})`);
    
    if (this.stats.failed > 0) {
      console.log(`❌ Falharam: ${this.stats.failed} arquivos`);
    }
    
    const successRate = Math.round((this.stats.uploaded / (this.stats.uploaded + this.stats.failed)) * 100);
    console.log(`📊 Taxa de sucesso: ${successRate}%`);
    
    console.log(`\n🌐 URL: ${this.baseUrl}/${this.container}/${this.projectName}/index.html`);
    console.log(`🎛️  Vault: https://vault.globoi.com/p/especiais_svelte/storage/objects/${this.container}/${this.projectName}/`);
